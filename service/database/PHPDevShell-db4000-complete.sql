...
CREATE TABLE `_db_core_node_items` (
  ...
  CONSTRAINT FOREIGN KEY (`plugin`) REFERENCES _db_core_plugin_activation (`plugin_folder`)
    ON UPDATE CASCADE,
  CONSTRAINT FOREIGN KEY (`theme_id`) REFERENCES _db_core_themes (`theme_id`)
    ON UPDATE CASCADE
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
...
CREATE TABLE `_db_core_users` (
  ...
  CONSTRAINT FOREIGN KEY (`user_role`) REFERENCES _db_core_user_roles (`user_role_id`)
    ON UPDATE CASCADE
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
...
CREATE TABLE `_db_core_cron` (
  ...
  PRIMARY KEY (`node_id`),
  CONSTRAINT FOREIGN KEY (`node_id`) REFERENCES _db_core_node_items (`node_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE
)
  ENGINE = InnoDB
...
CREATE TABLE `_db_core_filter` (
  ...
  CONSTRAINT FOREIGN KEY (`user_id`) REFERENCES _db_core_users(`user_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT FOREIGN KEY (`node_id`) REFERENCES _db_core_node_items(`node_id`)
    ON DELETE CASCADE
...
CREATE TABLE `_db_core_plugin_classes` (
  ...
  CONSTRAINT FOREIGN KEY (`plugin_folder`) REFERENCES _db_core_plugin_activation (`plugin_folder`)
    ON UPDATE CASCADE
    ON DELETE CASCADE
)
  ENGINE = InnoDB
...
INSERT INTO `_db_core_plugin_classes` (class_name, alias, plugin_folder, enable, rank) VALUES
('views', 'PHPDS_views', 'Mustache', '1', '1');
INSERT INTO `_db_core_plugin_classes` (class_name, alias, plugin_folder, enable, rank) VALUES
('pluginFactory', 'PluginManager_pluginFactory', 'PluginManager', '1', '1');
INSERT INTO `_db_core_plugin_classes` (class_name, alias, plugin_folder, enable, rank) VALUES
('pluginRepository', 'PluginManager_pluginRepository', 'PluginManager', '1', '1');
INSERT INTO `_db_core_plugin_classes` (class_name, alias, plugin_folder, enable, rank) VALUES
('nodeHelper', 'NodeHelper_nodeHelper', 'NodeHelper', '1', '1');
...
('PHPDS_allow_remember', '1', 'Should users be allowed to login with remember.');
INSERT INTO `_db_core_settings` VALUES
('PHPDS_banned_role', '4', 'The banned role. No access allowed.');
INSERT INTO `_db_core_settings` VALUES
('PHPDS_date_format', 'F j, Y, g:i a O', 'Date format according to DateTime function of PHP.');
INSERT INTO `_db_core_settings` VALUES
('PHPDS_date_format_short', 'Y-m-d', 'Shorter date format.');
INSERT INTO `_db_core_settings` VALUES
...
INSERT INTO `_db_core_settings` VALUES
('PHPDS_loginandout', 'login', 'The page to use to log-in and log-out.');
INSERT INTO `_db_core_settings` VALUES
('PHPDS_login_message', '', 'a Default message to welcome users loging in.');
INSERT INTO `_db_core_settings` VALUES
('PHPDS_meta_description', 'Administrative user interface based on PHPDS and other modern technologies.', '');
INSERT INTO `_db_core_settings` VALUES
...
INSERT INTO `_db_core_settings` VALUES
('PHPDS_root_id', '1', 'Root User.');
INSERT INTO `_db_core_settings` VALUES
('PHPDS_root_role', '1', 'Root Role.');
INSERT INTO `_db_core_settings` VALUES
('PHPDS_split_results', '15', 'When viewing paged results, how many results should be shown.');
INSERT INTO `_db_core_settings` VALUES
...
INSERT INTO `_db_core_settings` VALUES
('PHPDS_system_timezone', 'UTC', 'Timezone.');
...
CREATE TABLE `_db_core_user_role_permissions` (
  ...
  CONSTRAINT FOREIGN KEY (`user_role_id`) REFERENCES _db_core_user_roles (`user_role_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT FOREIGN KEY (`node_id`) REFERENCES _db_core_node_items (`node_id`)
    ON DELETE CASCADE
...
INSERT INTO _db_core_users
  (user_id, user_display_name, user_name, user_password, user_email,
   user_role, date_Registered, language, timezone, region)
VALUES ('1', 'Root User', 'root', '63a9f0ea7bb98050796b649e85481845',
        'admin@phpdevshell.org', '1', '1366524462', 'en', 'UTC', 'US');
-- Add extra custom settings;
INSERT INTO _db_core_settings VALUES
  ('PHPDS_crypt_key', 'ASKjjFHS223DjjLLPSfjsaDiEED', 'Crypt Key');
INSERT INTO _db_core_settings VALUES
  ('PHPDS_from_email', 'admin@phpdevshell.org', 'Where emails are shows to be sent from');
INSERT INTO _db_core_settings VALUES
  ('PHPDS_scripts_name_version', 'PHPDevShell', 'The name of the newly launched application');
INSERT INTO _db_core_settings VALUES
  ('PHPDS_setting_admin_email', 'admin@phpdevshell.org', 'Administrator email');
INSERT INTO _db_core_settings VALUES
  ('PHPDS_db_version', 4000, 'PHPDevShell Database Version');
-- That's it.
