...
class pagination extends PHPDS_dependant
{
	...
	public function searchFilter()
	{
		...
		} else if (!empty($_SESSION['filter_search']) && empty($this->configuration['user_id'])) {
			$this->filter['filter_search'] = $_SESSION['filter_search'];
			$this->filter['filter_order'] = $_SESSION['filter_order'];
			$this->filter['filter_by'] = $_SESSION['filter_by'];
		}
		// Lets save the search string to the database.
		if (!empty($_POST['search']) || !empty($_GET['order'])) {
			// Set search filter string...
			if (!empty($_POST['search_field'])) {
				$this->filter['filter_search'] = $_POST['search_field'];
			} else if (isset($_POST['search_field'])) {
				$this->filter['filter_search'] = '';
			} else {
				$this->filter['filter_search'] = '';
			}
			// Set filter order...
			if (!empty($_GET['order'])) {
				$this->filter['filter_order'] = $_GET['order'];
			...
			if ($this->configuration['user_id'] != 0) {
				...
				$_SESSION['filter_search'] = $this->filter['filter_search'];
				$_SESSION['filter_order'] = $this->filter['filter_order'];
				$_SESSION['filter_by'] = $this->filter['filter_by'];
			}
		}
	}
	...
	public function finalQuery($params)
	{
		...
		if (empty($this->limitCount)) {
			...
			// Assign limit count. //////////////////////////////////////////////////////////////////////////////////////////////////////
            if (! empty($config['split_results'])) {
			    $this->limitCount = $config['split_results'];
            } else {
                $this->limitCount = 10;
            }
		}
		...
		(isset($_GET['p']) && $_GET['p'] != false) ? $this->currentPage = $_GET['p'] : $this->currentPage = 1;
	...
	public function filterQuery()
	{
		...
		if (is_array($this->columns)) {
			...
			if (strlen($this->filter['filter_search']) != 0) {
				// Remove last OR.
				$search_columns = PU_rightTrim($search_columns, ' OR ');
