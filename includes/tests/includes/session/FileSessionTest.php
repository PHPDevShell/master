...
require_once $testing_dir."lib/filesession.php";
...
class FileSessionTest extends PHPUnit_Framework_TestCase {
    protected $session;
    ...
    /**
     * @expectedException EFSNotStartedException
     * @group session
     * @group filesession
     */
    public function testFileSessionNotStartedException() {
        $config = array(
        ...
        $this->session = new FileSession($config);
        $this->session->start();
        ...
        // Flush the session for other tests to be able to run
        $this->session->started = true;
        $this->session->flush();
    }

    /**
     * @expectedException   EFSWriteableException
     * @group session
     * @group filesession
     */
    public function testFileSessionWriteDirPermException() {
     ...
     * @group filesession
     */
    public function testFileSessionInvalidConfig() {
        // Test file cache startup without any config specified
        $this->session = new FileSession();
        $this->assertFalse($this->session->start());
        unset($this->session);

        // Test without session_write_dir setting specified
        $this->session = new FileSession(array('session_root_dir' => "/var/www/websentinel/"));
        $this->assertFalse($this->session->start());
        $this->session->flush();
    }
     ...
     * @group filesession
     */
    public function testFileSessionDisabled() {
     ...
     * @group filesession
     */
    public function testFileSessionFull() {
